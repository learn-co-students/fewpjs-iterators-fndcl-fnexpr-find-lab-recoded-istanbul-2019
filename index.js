...
function superbowlWin(record){
  //record.find(years =>{  return years.result === "W" });
  let records = record.find(function(years){ return  years.result === "W"});
  if(records){
    return records.year
  }else{
    return undefined
...
function superbowlWin(arr){
  arr.find(function(year) {
    let key = Object.keys(year);
    for(let i = 0; i < key.length ; i++){
      let keyValues =  key[i];
      let years = year[keyValues]
      if(years === "1969"){
       return years
      }
    }
